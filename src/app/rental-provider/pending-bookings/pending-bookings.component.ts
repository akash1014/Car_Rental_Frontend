...
import { ICustomWindow, RazorService } from 'src/app/razor-pay.service';
...
export class PendingBookingsComponent implements OnInit {
  ...
  constructor(private rental_service:RentalProviderService,private razorparService: RazorService,private zone: NgZone,private Router: Router) {
  ...
  razorPayHandler(res: any) {
    ...
    this.zone.run(() => {
      ...
      this.rental_service.generate_bills(this.GLOBAL_FORMDATA).subscribe((res:any)=>{
  ...
  generate_bill(id:any,rent:any,start_date:any,end_date:any){
    ...
    let NO_OF_DAYS=diff/(1000*3600*24);
    let total=NO_OF_DAYS*rent;
    let GST=13*(total)/100;
    let grandtotal=total+GST;

//multiplied by 100 as razorpay divides amount by 100 on its own
    this.razorPayOptions.amount = grandtotal*100;
    this.GLOBAL_FORMDATA={'booking_id':id};
    // @ts-ignore
  ...
  fetch_bookings(){
    this.rental_service.get_pending_bookings().subscribe((res:any)=>{
      if(res.success!=undefined){
