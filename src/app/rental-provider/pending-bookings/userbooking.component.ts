...
export class UserbookingComponent implements OnInit {
  ...
  // Initialize Razorpay
  initPay(data: any):void {
    this.formTextData = data;


    this.timediff = this.diff(data.fromtime, data.totime);
    console.log(this.timediff);
    this.formTextData['timediff'] = this.timediff;
    this.formTextData['id'] = this.id;

    this.price = this.price * this.timediff;
    this.razorPayOptions.amount = this.price * 100;
    // this.price = this.price * 100;
     console.log(this.price);

    let formdata = new FormData();
    formdata.append('timediff', this.timediff);
  ...
  razorPayHandler(res: any) {
    ...
    this.zone.run(() => {
      ...
      this.http.post(this.url + 'bookingform', this.formTextData).subscribe((res) => {
        console.log(res);
      });
    });
  }

  // TO CALCULATE TIME DIFFERENCE BETWEEN TIME FROM - TO TIME
  diff(from: any, to: any): any {
  ...
  // TO FILL THE FORM TO BOOK SERVICE
  bookingform(data: any): void {
    this.timediff = this.diff(data.fromtime, data.totime);
    ...
    this.userservice.bookingform(formdata).subscribe((res: any) => {
      if (res.insert) {
        this.bookclass = 'alert alert-success col-md-4';
        this.result = 'Booking is done';
      }
    });
  }
  ...
  ngOnInit(): void {
    ...
    this.userservice.showsp(this.id).subscribe((res: any) => {
      console.log(res.show);
      if (res.show) {
            console.log(this.id);
            this.row = res.show;
            this.price = this.row[0].payperhour;
            // console.log(this.price);
      }
    });
  }
